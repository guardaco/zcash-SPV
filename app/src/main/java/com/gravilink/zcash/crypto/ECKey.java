...
public class ECKey {
  ...
  public static ECKey fromPrivate(byte[] privKeyBytes, boolean isCompressed) {
    BigInteger privKey = new BigInteger(1, privKeyBytes);
    ECPoint point = publicPointFromPrivate(privKey);
    return new ECKey(privKey, getPointWithCompression(point, isCompressed));
  }
  ...
  public static ECKey publicKeyFromHex(String key) throws ZCashException {
    byte[] decoded = Base58.decode(key.substring(3, key.length()));
    byte[] pubKeyDecoded = Arrays.copyOfRange(decoded, 0, decoded.length - 4);
    byte[] checksum = Arrays.copyOfRange(decoded, decoded.length - 4, decoded.length);
    ECKey pubKey = ECKey.fromPublicOnly(pubKeyDecoded);
    byte[] calculatedChecksum = Utils.calculateChecksum(pubKeyDecoded);
    for (int i = 0; i < calculatedChecksum.length; i++) {
      if (checksum[i] != calculatedChecksum[i]) {
        throw new ZCashException("Invalid source address checksum.");
      }
    }
  ...
  public BigInteger getPrivKey() throws ZCashException {
    if (this.priv == null) {
      throw new ZCashException("Missing private key.");
    } else {
      return this.priv;
    }
  }
